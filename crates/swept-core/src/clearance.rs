...
//! contact points against everything, curbs included — a body may overhang a
//! curb, a tyre may not leave what it can roll on. The reverse test walks
...
impl ClearanceField {
    ...
    pub fn new(scene: &Scene, vehicle: &Vehicle) -> Self {
        // Strictly taller blocks: a curb exactly at the ground clearance is
        ...
        // Only blocking obstacles need the corner test: a curb corner inside
    ...
    ///
    /// Reported rather than penalised. A bumper crossing a sidewalk is legal
...
mod tests {
    ...
    /// A scene with a sidewalk the body can pass over.
    fn low_curb_scene() -> Scene {
        let mut scene = wide_scene();
        scene.curb_height = 0.12;
        scene
    }

    /// A pose with the nose over the sidewalk and every wheel off it.
    ///
    /// **Only an overhang can overhang.** The wheels sit at the corners of the
    /// body, so the flank is over a curb exactly when a tyre is — which is
    /// physically right, a wheel arch following its tyre to within a couple of
    /// centimetres. What can pass over a curb is therefore what sticks out
    /// beyond an axle: the front overhang, or the rear.
    ///
    /// Here the vehicle points into the yard on `wide_scene`, whose sidewalk
    ...
    /// -0.77, which is over the sidewalk. `x = -6` is clear of the dropped
    /// curb, which spans -1.60 to 1.60.
    fn overhanging_pose() -> Pose {
        Pose::new(-6.0, -4.2, Radians::from_degrees(90.0))
    }

    #[test]
    fn a_curb_lower_than_the_ground_clearance_does_not_stop_the_body() {
        let vehicle = lbx();
        let over = ClearanceField::new(&low_curb_scene(), &vehicle);
        assert_ne!(
            over.at(overhanging_pose()),
            Clearance::Collision,
            "a 12 cm curb passes under an 18 cm ground clearance"
        );
    }

    #[test]
    fn the_same_pose_is_a_collision_when_the_curb_is_a_wall() {
    ...
    #[test]
    fn a_low_curb_still_stops_a_wheel() {
        // Straddling the curb line, along the road. The near-side wheels land
        // at y = -0.09, up on the sidewalk, while the off-side pair stay at
        // -1.91 on the carriageway. The body may fly over a curb; a tyre may
        // not leave what it can roll on, and that alone must refuse this.
        let field = ClearanceField::new(&low_curb_scene(), &lbx());
    ...
    fn a_wall_taller_than_the_ground_clearance_stops_everything() {
        let mut scene = wide_scene();
        scene.curb_height = 0.40;
        let field = ClearanceField::new(&scene, &lbx());
        assert_eq!(field.at(overhanging_pose()), Clearance::Collision);
    }

    #[test]
    fn a_curb_exactly_at_the_ground_clearance_is_overhung() {
        ...
        // serve nobody.
        let mut scene = wide_scene();
        scene.curb_height = 0.18;
        let field = ClearanceField::new(&scene, &lbx());
        assert_ne!(field.at(overhanging_pose()), Clearance::Collision);
    }

    #[test]
    fn a_pose_over_the_sidewalk_is_reported_as_overhanging() {
        let field = ClearanceField::new(&low_curb_scene(), &lbx());
    ...
    #[test]
    fn nothing_overhangs_a_scene_whose_curb_is_a_wall() {
    ...
    fn an_overhung_obstacle_contributes_no_distance() {
        // The subtle half of the rule. If a curb the body flies over still
        ...
        match field.at(overhanging_pose()) {
            ...
            Clearance::Collision => panic!("the body overhangs this curb"),
        }
    }
    ...
    fn catches_an_obstacle_corner_inside_the_body() {
        ...
        // sidewalk. Its bumper centres pass at y = 0.275, clear of a wall only
